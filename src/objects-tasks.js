...
function compareObjects(obj1, obj2) {
  ...
  return res && Object.keys(obj1).length === Object.keys(obj2).length;
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    switch (bill) {
      case 25:
        bills[25] = bills[25] !== undefined ? bills[25] + 1 : 1;
        break;
      case 50:
        if (!bills[25]) return false;
        bills[25] -= 1;
        bills[50] += 1;
        break;
      case 100:
        ...
        return false;
      default:
        throw new Error('Such bill values are not supported');
    }
  }
  return true;
}
...
function group(array, keySelector, valueSelector) {
  return array.reduce((res, obj) => {
    const value = res.get(keySelector(obj));
    if (value) res.set(keySelector(obj), [...value, valueSelector(obj)]);
    else res.set(keySelector(obj), [valueSelector(obj)]);
    return res;
  }, new Map());
}
...
class SimpleSelector {
  constructor(initialValue) {
    this.selectors = [];
    if (initialValue) {
      this.selectors.push(initialValue);
    }
    this.selectorsOrder = {
      element: 0,
      id: 1,
  ...
  element(value) {
    ...
    if (this.selectorsOrder.element < this.currentOrder) {
      throw new Error(
    ...
    this.currentOrder = this.selectorsOrder.element;
    return this;
  }
  ...
  id(value) {
    ...
    if (this.selectorsOrder.id < this.currentOrder) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    this.selectors.push(`#${value}`);
    this.idAdded = true;
    this.currentOrder = this.selectorsOrder.id;
    return this;
  }

  class(value) {
    if (this.selectorsOrder.class < this.currentOrder) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    this.selectors.push(`.${value}`);
    this.currentOrder = this.selectorsOrder.class;
    return this;
  }

  attr(value) {
    if (this.selectorsOrder.attr < this.currentOrder) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    this.selectors.push(`[${value}]`);
    this.currentOrder = this.selectorsOrder.attr;
    return this;
  }

  pseudoClass(value) {
    if (this.selectorsOrder.pseudoClass < this.currentOrder) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    this.selectors.push(`:${value}`);
    this.currentOrder = this.selectorsOrder.pseudoClass;
    return this;
  }

  pseudoElement(value) {
    if (this.selectorsOrder.pseudoElement < this.currentOrder) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    ...
    this.currentOrder = this.selectorsOrder.pseudoElement;
    return this;
  }
...
const cssSelectorBuilder = {
  ...
  combine(selector1, combinator, selector2) {
    let str1 = selector1;
    let str2 = selector2;
    if (selector1 instanceof SimpleSelector) {
      str1 = selector1.stringify();
    }
    if (selector2 instanceof SimpleSelector) {
      str2 = selector2.stringify();
    }
    return new SimpleSelector(`${str1} ${combinator} ${str2}`);
  },
};
